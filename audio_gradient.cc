...
// float get_scale(int pos, int size) {
//   std::string str = smcc::func_name() + "(" + std::to_string(pos)  + "., " + std::to_string(size) + ".)";
//   smcc::AST ast2(str.c_str(), str.size());
//   smcc::CodeGen codegen2(ast2);
//   return ((double *)ast2.tree_v2()->kids()[0]->kids()[0]->var())[0];
// }

const char *func = "double main(int pos, int size) { if (pos < 0.3 * size) { return 0.; } else if (pos < 0.7 * size) { return 1; } else { return 0.; } } ";

float get_scale(double pos, double size) {
  if (pos < 0.3 * size) {
    return 0.1;
  }
...
void tune(uint8_t **data, int nb_samples, int nb_channels, AVSampleFormat format, double pos, double size) {
  ...
  // float scale = get_scale(pos, size);
  smcc::reset();
  std::vector<double> args = std::vector<double>{pos, size};
  float scale = smcc::call("main", args);
  for (i = 0; i < nb_samples; i++) {
    for (plane = 0; plane < nb_planes; plane++) {
      pcm = (float *)data[plane];
      if (pcm[i] >= 0) {
        // pcm[i] = std::min(std::max(pow(pcm[i], 1 / scale), -1.f), 1.f);
        pcm[i] = std::min(std::max(pcm[i] * scale, -1.f), 1.f);
      }
      else {
        // pcm[i] = -std::min(std::max(pow(abs(pcm[i]), 1 / scale), -1.f), 1.f);
        pcm[i] = -std::min(std::max(abs(pcm[i]) * scale, -1.f), 1.f);
      }
    }
  }
}
...
class AudioGradient {
 public:
  AudioGradient() = default;

  int run(const char *input, const char *output, const char *func) {
    auto t1 = time();
  
    smcc::ReaderMem reader(func, strlen(func));
    smcc::AST ast(&reader);
    ast.parse();
    ...
    while (swr_get_out_samples(swr_ctx, 0) > audio_output_frame_size) {
      ...
      tune(buffer, samples, output_av_codec_ctx->channels, output_av_codec_ctx->sample_fmt, pos * 1000. / output_av_codec_ctx->sample_rate, nframes * 1000. / output_av_codec_ctx->sample_rate);
      pos += samples;
    ...
    auto t2 = time();
    fprintf(stderr, "AudioGradient Time ... %f ms\n", t2 - t1);
...
int audio_gradient(const char *input, const char *output, const char *func) {
  // 91.650146 ms For c++
  fprintf(stderr, "%s AudioGradient ... %s, %s\n", VERSION, input, output);
  AudioGradient read;
  int ok = read.run(input, output, func);
  fprintf(stderr, "AudioGradient ok ... %d\n", ok);
  return ok;
}
...
int main(int argv, char *args[]) {
  ...
  // float (*func)(int, int) = [](int pos, int size) {
  //   if (pos < 0.3 * size) {
  //     return 0.1f;
  //   }
  //   else if (pos < 0.7 * size) {
  ...
  int ok = audio_gradient(input, output, func);
  fprintf(stderr, "ok ... %d\n", ok);
  while (mod) {
    ok = audio_gradient(input, output, func);
    fprintf(stderr, "ok ... %d\n", ok);
  }
}
#endif  // WITH_MAIN
